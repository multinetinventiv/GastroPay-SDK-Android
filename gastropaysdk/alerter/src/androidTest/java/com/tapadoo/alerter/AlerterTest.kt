...
class AlerterTest {
    ...
    fun testShow() {
        ...
        val childCount =
            (activityRule.activity.findViewById<View>(android.R.id.content).rootView as ViewGroup).childCount
    ...
    fun testBuilderStringsRes() {
        ...
        Assert.assertSame(
            alert?.findViewById<TextView>(R.id.tvTitle)?.text,
            activityRule.activity.getString(R.string.lib_name)
        )
        Assert.assertSame(
            alert?.findViewById<TextView>(R.id.tvTitle)?.text,
            activityRule.activity.getString(R.string.msg_no_alert_showing)
        )
    }
    ...
    fun testBuilderIcon() {
        ...
        Assert.assertNotSame(
            alert?.findViewById<ImageView>(R.id.ivIcon)?.drawable?.constantState,
    ...
    fun testBuilderBackground() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            Assert.assertEquals(
                (alert?.findViewById<ViewGroup>(R.id.llAlertBackground)?.background as ColorDrawable).color.toLong(),
                ContextCompat.getColor(activityRule.activity, android.R.color.darker_gray).toLong()
            )
        }
    }
    ...
    fun testBuilderOnClickListener() {
        ...
        Assert.assertTrue(
            alert1?.findViewById<ViewGroup>(R.id.llAlertBackground)?.hasOnClickListeners() ?: false
        )
        ...
        Assert.assertTrue(
            alert3?.findViewById<ViewGroup>(R.id.llAlertBackground)?.hasOnClickListeners() ?: false
        )
    }
}
